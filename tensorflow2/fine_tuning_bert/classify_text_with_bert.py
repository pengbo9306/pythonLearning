...
# 接下来，您将使用该text_dataset_from_directory实用程序来创建带标签的tf.data.Dataset。
...
# 让我们重新加载模型，以便可以与仍在内存中的模型并排尝试。
reloaded_model = tf.saved_model.load(saved_model_path)


# 在这里，您可以在所需的任何句子上测试模型，只需将其添加到以下示例变量中即可。
...
# 如果要在TF Serving上使用模型，请记住，它将通过其命名签名之一调用SavedModel。在Python中，您可以如下进行测试：
